...
function AppSettings({ s, patch, t }: { s: AppState, patch: any, t: any }) {
  ...
  const pullCloudToLocal = async (skipConfirm = false) => {
    if (!skipConfirm && !confirm('Pull cloud data to this desktop? This will replace local app data with cloud data. Take a backup first if unsure.')) return
    setSyncing(true)
