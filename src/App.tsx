...
function Billing({ s, patch, t }: { s: AppState, patch: (fn: (s: AppState) => AppState) => void, t: any }) {
  ...
  const popular = [...s.vegetables].filter(v => v.active).sort((a, b) => b.stock - a.stock).slice(0, 12)

  const addItem = (v: Vegetable) => {
    setCart(items => items.some(i => i.vegetableId === v.id)
      ? items.map(i => i.vegetableId === v.id ? { ...i, qty: Number((i.qty + 1).toFixed(2)) } : i)
      : [...items, { vegetableId: v.id, name: v.name, hindiName: v.hindiName, unit: v.unit, qty: 1, rate: v.sellingRate, discount: 0 }])
    setQuery('')
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') { e.preventDefault(); if (confirm('Clear current bill?')) setCart([]) }
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [cart, totals.total, mode, paid, customerId, customerName, customerPhone])

  return <div className="billing-grid"><div className="space">
    <Card className="pad"><label>{t.searchVegetable} <small>Shortcuts: F2 search, F4 cash, F5 save, F6 print, Esc clear</small></label><Input ref={searchRef} autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder="Tomato / टमाटर / TOM" />{filtered.length > 0 && <div className="search-results">{filtered.map(v => <button key={v.id} onClick={() => addItem(v)}><span><b>{v.hindiName} / {v.name}</b><small>{v.stock} {v.unit} • {money(v.sellingRate)}/{v.unit}</small></span><b>+</b></button>)}</div>}</Card>
    <Card className="pad"><h2>Quick Billing</h2><div className="quick-grid">{popular.map(v => <button key={v.id} onClick={() => addItem(v)}><b>{v.hindiName || v.name}</b><small>{money(v.sellingRate)}/{v.unit} • {v.stock} {v.unit}</small></button>)}</div></Card>
  ...
  </div><div className="space"><Card className="pad"><h2>{t.billing}</h2><Select value={customerId} onChange={e => chooseCustomer(e.target.value)}><option value="">{t.cashCustomer}</option>{s.customers.map(c => <option key={c.id} value={c.id}>{c.name} - {money(c.balance)}</option>)}</Select><Input placeholder={t.customer} value={customerName} onChange={e => setCustomerName(e.target.value)}/><Input placeholder={t.phone} value={customerPhone} onChange={e => setCustomerPhone(e.target.value)}/><Select value={mode} onChange={e => setMode(e.target.value as PaymentMode)}>{paymentModes.map(m => <option key={m}>{m}</option>)}</Select><Input type="number" placeholder={t.paid} value={paid} onChange={e => setPaid(number(e.target.value))}/></Card><Card className="pad"><Line label="Subtotal" value={money(totals.subtotal)}/><Line label={t.discount} value={money(totals.discount)}/><Line label="Round Off" value={money(totals.roundOff)}/><div className="grand"><span>{t.total}</span><b>{money(totals.total)}</b></div><Line label={t.due} value={money(Math.max(0, totals.total - (mode === 'Credit' ? 0 : (paid || totals.total))))}/><Button className="full" onClick={() => save(false)}>{t.save}</Button><Button className="full" variant="secondary" onClick={() => save(true)}>{t.savePrint}</Button></Card></div></div>
}
