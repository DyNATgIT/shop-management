...
function AppSettings({ s, patch, t }: { s: AppState, patch: any, t: any }) {
  ...
  const checkCloudCounts = async () => {
    setCloudTest('Checking cloud row counts...')
    try {
      if (!settings.cloudAccessToken || !settings.cloudShopId) throw new Error('Sign in and connect cloud shop first')
      ...
      for (const table of tables) {
        const res = await fetch(`${base}/rest/v1/${table}?shop_id=eq.${settings.cloudShopId}&select=id`, { method: 'HEAD', headers: { ...cloudHeaders(settings.cloudAccessToken), Prefer: 'count=exact' } })
