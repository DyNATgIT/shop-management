  ...
  return <div className="app">
    <header className="topbar">
      <div className="head-row">
        ...
        <div className="head-actions"><Button variant="secondary" onClick={() => patch(s => ({ ...s, settings: { ...s.settings, language: s.settings.language === 'en' ? 'hi' : 'en' } }))}><Languages size={16}/>{state.settings.language === 'en' ? 'हिन्दी' : 'English'}</Button>{state.settings.ownerPinEnabled && ownerUnlocked && <Button variant="secondary" onClick={() => setOwnerUnlocked(false)}>Lock Owner</Button>}<span className="offline">Offline</span></div>
      </div>
...
function PinGate({ settings, onUnlock }: { settings: any, onUnlock: () => void }) {
...
function AppSettings({ s, patch, t, onLockOwner }: { s: AppState, patch: any, t: any, onLockOwner?: () => void }) {
  ...
  const runCloudHealthCheck = async () => {
    ...
    try {
      ...
      const requiredTables = ['shops', 'shop_users', 'vegetables', 'customers', 'suppliers', 'sales', 'sale_items', 'purchases', 'purchase_items', 'payments', 'expenses', 'stock_logs', 'sync_devices']
