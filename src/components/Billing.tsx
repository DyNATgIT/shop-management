...
import { calcCart, id, makeStockLog, money, now, number, paymentModes, saleUnitsFor, toStockQty } from '../lib/store'
...
export default function Billing({ s, patch, t }: { s: AppState, patch: (fn: (s: AppState) => AppState) => void, t: any }) {
  ...
  const selectedStockQty = selectedVeg ? toStockQty(selectedQty, selectedUnit, selectedVeg.unit) : 0
  const selectedAmount = selectedVeg ? selectedStockQty * selectedVeg.sellingRate - selectedDiscount : 0
  const selectedUnitOptions = selectedVeg ? saleUnitsFor(selectedVeg.unit) : []
  const quickQty = selectedUnit === 'kg' ? [0.25, 0.5, 1, 2, 5] : (selectedUnit === 'g' || selectedUnit === 'gram' || selectedUnit === 'grams') ? [100, 250, 500, 1000] : [1, 2, 5, 10]

  const openQty = (v: Vegetable) => {
    setSelectedVeg(v)
    setSelectedQty(v.unit === 'kg' ? 1 : (v.unit === 'g' || v.unit === 'gram' || v.unit === 'grams') ? 250 : 1)
    setSelectedUnit(v.unit || 'kg')
    setSelectedDiscount(0)
    setQuery('')
  }
  const addItem = (v: Vegetable, qty = 1, discount = 0, saleUnit = v.unit) => {
    const stockQty = toStockQty(qty, saleUnit, v.unit)
    const errors = [validatePositive(qty, `${v.name} quantity`), validatePositive(stockQty, `${v.name} stock quantity`)].filter(Boolean)
    if (discount < 0) errors.push('Discount cannot be negative.')
    if (discount > stockQty * v.sellingRate) errors.push('Discount cannot be greater than item amount.')
    if (showValidation(errors)) return
    setCart(items => items.some(i => i.vegetableId === v.id && i.unit === saleUnit)
      ? items.map(i => i.vegetableId === v.id && i.unit === saleUnit ? { ...i, qty: Number((i.qty + qty).toFixed(3)), stockQty: Number(((i.stockQty ?? i.qty) + stockQty).toFixed(3)), discount: Number((i.discount + discount).toFixed(2)) } : i)
      : [...items, { vegetableId: v.id, name: v.name, hindiName: v.hindiName, unit: saleUnit, stockUnit: v.unit, qty, stockQty, rate: v.sellingRate, discount }])
    setSelectedVeg(null)
    setSelectedQty(1)
    setSelectedUnit(v.unit || 'kg')
    setSelectedDiscount(0)
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter' && selectedVeg) { e.preventDefault(); addItem(selectedVeg, selectedQty, selectedDiscount, selectedUnit) }
    }
  ...
  return <div className="billing-grid"><div className="space">
    ...
    <Card><div className="table-wrap"><table><thead><tr><th>{t.name}</th><th>{t.qty}</th><th>{t.rate}</th><th>{t.discount}</th><th>{t.amount}</th><th></th></tr></thead><tbody>{cart.map((item, idx) => <tr key={item.vegetableId}><td><b>{item.hindiName}</b><small>{item.name}</small></td><td><Input type="number" step="0.01" value={item.qty} onChange={e => setCart(c => c.map((x, n) => n === idx ? { ...x, qty: number(e.target.value), stockQty: toStockQty(number(e.target.value), x.unit, x.stockUnit || x.unit) } : x))}/><small>{item.unit}</small></td><td><Input type="number" value={item.rate} onChange={e => setCart(c => c.map((x, n) => n === idx ? { ...x, rate: number(e.target.value) } : x))}/></td><td><Input type="number" value={item.discount} onChange={e => setCart(c => c.map((x, n) => n === idx ? { ...x, discount: number(e.target.value) } : x))}/></td><td><b>{money((item.stockQty ?? item.qty) * item.rate - item.discount)}</b></td><td><button className="icon danger-text" onClick={() => setCart(c => c.filter((_, n) => n !== idx))}><Trash2 size={16}/></button></td></tr>)}{!cart.length && <tr><td colSpan={6} className="empty">{t.noData}</td></tr>}</tbody></table></div></Card>
  ...
  {selectedVeg && <div className="modal-backdrop" onClick={() => setSelectedVeg(null)}><div className="qty-modal" onClick={e => e.stopPropagation()}><div className="modal-title"><div><h2>{selectedVeg.hindiName || selectedVeg.name}</h2><p>{selectedVeg.name} • {money(selectedVeg.sellingRate)}/{selectedVeg.unit} • Stock {selectedVeg.stock} {selectedVeg.unit}</p></div><button onClick={() => setSelectedVeg(null)}>×</button></div><label>{t.qty}</label><Input autoFocus type="number" step="0.01" value={selectedQty} onChange={e => setSelectedQty(number(e.target.value))}/><label>Billing Unit</label><select className="input" value={selectedUnit} onChange={e => setSelectedUnit(e.target.value)}>{selectedUnitOptions.map(u => <option key={u} value={u}>{u}</option>)}</select><small>Stock will reduce by {selectedStockQty.toLocaleString('en-IN', { maximumFractionDigits: 3 })} {selectedVeg.unit}</small><div className="qty-chips">{quickQty.map(q => <button key={q} onClick={() => setSelectedQty(q)}>{q} {selectedUnit}</button>)}</div><label>{t.discount}</label><Input type="number" value={selectedDiscount} onChange={e => setSelectedDiscount(number(e.target.value))}/><div className="preview-total"><span>{t.amount}</span><b>{money(selectedAmount)}</b></div><div className="modal-actions"><Button variant="secondary" onClick={() => setSelectedVeg(null)}>{t.cancel}</Button><Button onClick={() => addItem(selectedVeg, selectedQty, selectedDiscount, selectedUnit)}>{t.add}</Button></div></div></div>}
  </div>
}
