...
export default function AppSettings({ s, patch, t, onLockOwner }: { s: AppState, patch: any, t: any, onLockOwner?: () => void }) {
  const [settings, setSettings] = useState(s.settings); const [expense, setExpense] = useState({ title: '', amount: 0, note: '' }); const [dbInfo, setDbInfo] = useState<any>(null); const [cloudTest, setCloudTest] = useState(''); const [cloudPassword, setCloudPassword] = useState(''); const [syncing, setSyncing] = useState(false); const [pinDraft, setPinDraft] = useState(''); const [inviteRole, setInviteRole] = useState<'manager' | 'staff'>('staff'); const [inviteCode, setInviteCode] = useState(''); const [teamMembers, setTeamMembers] = useState<any[]>([]); const [teamMessage, setTeamMessage] = useState('')
  const isDesktop = Boolean(window.desktopApp?.isDesktop)
  const isWebsite = !isDesktop
  const tabOptions = [
    { id: 'dashboard', label: 'Dashboard' },
    { id: 'billing', label: 'Billing' },
    { id: 'inventory', label: 'Vegetables' },
    { id: 'rates', label: 'Daily Rates' },
    { id: 'purchases', label: 'Mandi Purchase' },
  ...
  const toggleStaffTab = (tabId: string) => {
    const current = settings.staffAllowedTabs?.length ? settings.staffAllowedTabs : ['dashboard', 'billing', 'customers', 'payments', 'wastage']
    const nextTabs = current.includes(tabId) ? current.filter(x => x !== tabId) : [...current, tabId]
    setSettings({ ...settings, staffAllowedTabs: nextTabs })
  }
  ...
  const loadTeamMembers = async () => {
    setTeamMessage('Loading team...')
    try {
      if (!settings.cloudShopId || !settings.cloudAccessToken) { setTeamMessage('Sign in and connect cloud shop first.'); return }
      const fresh = await refreshCloudSession(settings).catch(() => settings)
      if (fresh !== settings) { setSettings(fresh); saveCloudSettings(fresh) }
      const res = await fetch(`${settings.supabaseUrl.replace(/\/$/, '')}/rest/v1/rpc/list_shop_members`, {
        method: 'POST',
        headers: { apikey: fresh.supabaseAnonKey, Authorization: `Bearer ${fresh.cloudAccessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ p_shop_id: fresh.cloudShopId })
      })
      const data = await res.json().catch(() => null)
      if (!res.ok) { setTeamMessage(data?.message || data?.hint || `Load team failed: ${res.status}`); return }
      setTeamMembers(Array.isArray(data) ? data : [])
      setTeamMessage('Team loaded.')
    } catch (error) { setTeamMessage(`Load team failed: ${error instanceof Error ? error.message : 'Unknown error'}`) }
  }
  const updateTeamRole = async (userId: string, role: string) => {
    if (!confirm(`Change this user role to ${role}?`)) return
    setTeamMessage('Updating role...')
    try {
      const fresh = await refreshCloudSession(settings).catch(() => settings)
      const res = await fetch(`${settings.supabaseUrl.replace(/\/$/, '')}/rest/v1/rpc/update_shop_member_role`, {
        method: 'POST',
        headers: { apikey: fresh.supabaseAnonKey, Authorization: `Bearer ${fresh.cloudAccessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ p_shop_id: fresh.cloudShopId, p_user_id: userId, p_role: role })
      })
      const data = await res.json().catch(() => null)
      if (!res.ok) { setTeamMessage(data?.message || data?.hint || `Update failed: ${res.status}`); return }
      setTeamMessage('Role updated.')
      loadTeamMembers()
    } catch (error) { setTeamMessage(`Update failed: ${error instanceof Error ? error.message : 'Unknown error'}`) }
  }
