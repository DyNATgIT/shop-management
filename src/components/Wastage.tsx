...
import { showValidation, validatePositive } from '../lib/validation'
...
export default function Wastage({ s, patch, t }: { s: AppState, patch: any, t: any }) {
  ...
  const add = () => { const v = s.vegetables.find(x => x.id === vegetableId); if (!v) return alert('Select a vegetable'); if (showValidation([validatePositive(qty, 'Wastage quantity')].filter(Boolean))) return; patch((old: AppState) => ({ ...old, vegetables: old.vegetables.map(x => x.id === v.id ? { ...x, stock: Math.max(0, x.stock - qty), lastUpdated: now() } : x), stockLogs: [makeStockLog(v.id, v.name, 'WASTAGE', -qty, v.stock, note), ...old.stockLogs] })); setQty(0) }
